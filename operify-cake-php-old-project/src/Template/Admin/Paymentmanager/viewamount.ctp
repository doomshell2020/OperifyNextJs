...
<?php $k = 1;
$receivedTotal = array_sum(array_column($EmdRemarks, 'recive_amount'));
...
<div class="tableContainer">
    ...
    <div class="tableHeader">
        <p style="text-align:center;font-size:15px;"><b>Payment Details</b></p>
        <table>
            <tr>
                <td style="text-align:left;">
                <td><b>PO No.:</b> <?= h($EmdGuarantees['po_no']) ?></td>
                </td>
            </tr>
            <tr>
                <td><b>Particular's Name:</b> <?= h($EmdGuarantees['particular']) ?></td>
                <td><b>Date:</b> <?= date('d-m-Y', strtotime($EmdGuarantees['datefrom'])) ?></td>
                <td><b>Total Amount:</b> <?= number_format($EmdGuarantees['amount']) ?></td>
            </tr>
            <tr>
                <td><b>Invoice No.:</b> <?= h($EmdGuarantees['invoice']) ?></td>
                <td><b>Due Period:</b> <?= h($EmdGuarantees['due_period']) ?> Days</td>
                <td><b>Pending Amount:</b> <?= number_format($pendingAmount) ?></td>
    ...
    <div class="table-responsive" style="padding: 10px;">
                    ...
                    <tr>
                        ...
                        <td style="text-align:center;">
                            <?php if (!empty($Remarks['invoice_file'])) {
                                ...
                                $filePath = '/images/' . $db . '_image/payments/' . h($Remarks['invoice_file']);
                            ?>
        ...
        ?>
            <div style="margin-top: 15px; text-align: center;">
                <button type="button" onclick="document.getElementById('remarkForm').style.display='block'" class="btn btn-primary">+ Add Payment</button>
            </div>
        <?php } ?>
        <!-- Add Payment Form -->
        <div id="remarkForm" style="display: none; margin-top: 15px; border: 1px solid #ccc; padding: 15px; background-color: #f9f9f9;">
            <p style="text-align:center;font-size:15px;"><b>Add New Payment</b></p>

            <?= $this->Form->create(null, ['type' => 'file','class' => 'form-horizontal']) ?> <!-- updated to allow file upload -->
