...
foreach ($indentpoid as $detail) {
    ...
    $indentdetails = $this->comman->getindentdetails($detail['indent_id']);
    ...
    foreach ($indentdetails as $value) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('B' . $ii, $detail['indent_id'])->getStyle('B' . $ii)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('H' . $ii, ucfirst($detail['issued_name']));
...
$filename = "Indent_Summary_" . $date . ".xlsx";
