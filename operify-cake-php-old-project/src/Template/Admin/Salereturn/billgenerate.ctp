...
$branchdata = $this->Comman->branchsales($branch_request['branch_name']);  
$branchdata_detail = $this->Comman->branchsales_detail($branch_request['branch_name']);  
...
$approved_date  =  $branch_request['approved_date'];
...
<table width="100%" cellpadding="3px">
  ...
  <tr>
    ...
    <td style="width:10%; border:1px solid #ddd; text-align:left; font-size:7px; ">
    '.date('Y-m-d', strtotime($approved_date)).'
    </td>
    <td style="width:20%; border:1px solid #ddd; text-align:left; font-size:7px; font-weight:bold ">
    TRANSPORT MODE
    </td>
  ...
  <tr>
    ...
    $i=1; 
   // pr($branch_request); 
    foreach($branch_request['salesreturndetils'] as $key => $value){ //pr($value); die;
...
foreach($branch_request['salesreturndetils'] as $key=>$value){
  if($value['item_tax']){
...
$t =1; foreach($array_Data as $key=>$value){ //pr($value); die;
  ...
  $taxableprice = $this->Comman->billtaxdatasale($value,$id); 
  $tax_amountdata = $this->Comman->billgstsale($value,$id); 
  $total_amountdata = $this->Comman->billtotalamountsale($value,$id); 
