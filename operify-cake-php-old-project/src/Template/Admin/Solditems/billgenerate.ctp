...
$branchdata = $this->Comman->branchdataget($branch_name);  
$branchdata_detail = $this->Comman->branchdataget_detail($branch_name); 
...
$other_amt  =  $branch_request['other_amt'];
...
<table width="100%" cellpadding="3px">
  ...
  if($branch_request['manual_receipt_no']) {
  $html.='<tr>
    <td style="width:100%; border:1px solid #ddd; font-size:7px; text-align:left;  font-weight:bold; ">
    Manual Bill No : '.$branch_request['manual_receipt_no'].'
    </td>
  </tr>';
}

if(date('d-m-Y', strtotime($branch_request['manual_receipt_date']))!= "01-01-1970") {
$html.='<tr>
    <td style="width:100%; border:1px solid #ddd; font-size:7px; text-align:left;  font-weight:bold; ">
    Manual Bill Date : '.date('d-m-Y', strtotime($branch_request['manual_receipt_date'])).'
    </td>
  </tr>';
}
  $html.='<tr>
    ...
    $i=1; foreach($branch_request['solditemdetails'] as $key=>$value){
...
// taxable amount
  ...
  <tr>
    ...
    <td style="width:20%; border:1px solid #ddd; text-align:left; font-size:7px;">'.ucfirst(strtolower(trim($value['item_name']))).'</td>
    ...
    <td style="width:7%; border:1px solid #ddd; text-align:right;font-size:7px;">
    '.round($total_tax,2).'
    </td> 
  ...
  $final_total_amount_data = $other_amt + $totalamount_data;
  ...
  <tr>
    ...
    <td style="width:10%; border:1px solid #ddd; text-align:right;   ">
    '.sprintf('%.2f',round($final_total_amount_data)).'
    </td>
  </tr>
</table>
<br>
...
foreach($branch_request['solditemdetails'] as $key=>$value){
  if($value['item_tax']){
...
$t =1; foreach($array_Data as $key=>$value){ //pr($value); die;
  ...
  $taxableprice = $this->Comman->soldtaxdatanew($value,$id); 
  $tax_amountdata = $this->Comman->soldgst($value,$id); 
  $total_amountdata = $this->Comman->soldtotalamount($value,$id); 
...
$html.='<tr>
  ...
  <td width="16.66%" style="border:1px solid #ddd; font-size:7px; text-align:center;">'.sprintf('%.2f',$value).'</td>
  <td width="16.66%" style="border:1px solid #ddd; font-size:7px; text-align:center;">'.sprintf('%.2f',$taxableprice).'</td>';
