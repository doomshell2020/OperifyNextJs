...
$branch_request_id  =  $branch_request[0]['id'];
$pay_Date  =  $branch_request[0]['created'];
$customer_name  =  $branch_request[0]['student_purchasereturn']['student']['fname'];
$customer_mobile  =  $branch_request[0]['student_purchasereturn']['student']['mobile'];
if($branch_request['discount']){
$other_dis  =  $branch_request['discount'];
}else{
$other_dis  = 0;
}
...
<table width="100%" cellpadding="3px">
  ...
  <tr>
    ...
    <td style="width:20%; border:1px solid #ddd; font-size:7px; text-align:left;  ">
    '.$branch_request[0]['student_purchasereturn']['student']['address'].'
    </td>
  ...
  <tr>
    ...
    $i=1; foreach($branch_request as $key=>$value){ //pr($value); die;
   
         
        $gname=$this->Comman->findbillitemname($value['item_id']); 


    $total_amount =  $value['item_qty'] * $value['item_price'];
    $total_discount =  $value['item_qty'] * $value['discount'];
    $taxable_amount = $total_amount-$total_discount;
    $tax_amount = $taxable_amount*$gname[0]['tax_name']/100;
    $totalamount = $taxable_amount+$tax_amount;
 
    if($gname[0]['tax_name'] == 0){ 
      $total_tax =   "N/A"; 
      }else{ 
      $total_tax = $gname[0]['tax_name']."%";
      }
      $item_amount +=  $value['item_price']; 
      $item_qty +=  $value['item_qty']; 
      $discount +=  $value['discount']; 
      $taxable_amountdata += $total_amount-$total_discount;
      $tax_amountdata += $taxable_amount*$gname[0]['tax_name']/100;
      $totalamount_data += $taxable_amount+$tax_amount;

// taxable amount
    $itemtax_price=$value['item_price']*$gname[0]['tax_name']/100;
  ...
  <tr>
    ...
    <td style="width:20%; border:1px solid #ddd; text-align:left; font-size:7px;">'.ucfirst(strtolower(trim($gname[0]['item_name']))).'</td>
...
$tt_data  = sprintf('%.2f',round($totalamount_data)) - sprintf('%.2f',round($other_dis));
$html.='<tr>
    <td style="width:90%; border:1px solid #ddd; text-align:right; font-weight:bold;   ">
    Total Tax Amount
    </td>
...
foreach($branch_request as $key=>$value){
    $tax_collection=$this->Comman->findbilltax($value['item_id']); 
   // pr($tax_collection); die;
  if($tax_collection[0]['tax_name']){
    $tax_data[] = $tax_collection[0]['tax_name']; 
  }else{
    $tax_data[] = 0;
  }
