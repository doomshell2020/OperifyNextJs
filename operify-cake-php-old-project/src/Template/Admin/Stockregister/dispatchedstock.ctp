...
<div class="content-wrapper">
  <section class="content-header">
    <h1>
      Dispatched Item Name-<?php $getsize = $this->Comman->getsizename($additem['size_id']);
                            echo "<b>" . $additem['item_name'] . "</b>";
                            if ($getsize['id'] != 6) {
                              echo "(" . $getsize['size_name'] . ")";
                            } ?> </h1>
    <ol class="breadcrumb">
          ...
          <div class="box-body">
            <table id="example14" class="table table-bordered table-striped">
              <thead>
                <tr>
                  <th>S.No.</th>
                  <th>Indent ID / JC ID </th>
                  <th>Issue Quantity</th>
                  <th>Issue Date</th>
                  <th>Indent Status / JC Status</th>
              ...
              <tbody>
                ...
                if (isset($stockregister) && !empty($stockregister)) {
                  foreach ($stockregister as $intusr) {
                    ...
                    if (!empty($intusr['sub_contractors_id'])) {
                      $issue_date =  (!empty($intusr['issue_date']) ? $intusr['issue_date']->format('d-m-Y') : '');
                    } else {
                      $issue_date =  $intusr['created']->format('d-m-Y');
                    }
                ...
                ?>
                    <tr>
                      ...
                      <td> <?php echo $indent_id; ?>

                        <?php if (empty($intusr['sub_contractors_id'])) {  ?>

                          <a style="font-size: 20px;" target="_blank"
                            href="<?php echo ADMIN_URL; ?>indentpo/viewindentpopdf/<?php echo $indent['indent_id']; ?>"><i
                              class="fa fa-file-pdf-o" style="font-size: 20px;"></i>&nbsp;</a>
                        <?php   } ?>
                      </td>
                      <td> <?php echo $intusr['quantity']; ?><br>
                        <?php if (!empty($intusr['sub_contractors_id'])) {  ?>

                          <b>Sub Contractors Name:</b> <?php echo ucwords($subContractorsDetail['name']); ?><br>
                          <b>Mobile No.:</b> <?php echo ucwords($subContractorsDetail['mobile']); ?><br>
                          <b>Email:</b> <?php echo ucwords($subContractorsDetail['email']); ?><br>
                        <?php   } ?>
